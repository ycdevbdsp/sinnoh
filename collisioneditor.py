...
class CollisionEditor(QWidget):
    ...
    def __init__(self, collisionData, exData, placeData, mapOffsets, parent=None):
        super().__init__()
    ...
    def drawOverworldEventMap(self, map):
        ...
        for key in self.PlaceData:
            ...
            pdx = int(pd['Position']['x'] - (x * 32)) * self.CellSize['width']
            pdy = int(pd['Position']['y'] - (y * 32)) * self.CellSize['height']
            print(f"{floor(pdx/self.CellSize['width'])}{floor(pdy/self.CellSize['height'])}")
            self.PlaceDataCells[f"{floor(pdx/self.CellSize['width'])}{floor(pdy/self.CellSize['height'])}"] = pd
